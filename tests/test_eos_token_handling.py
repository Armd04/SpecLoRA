...
def test_eos_token_from_list():
    ...
    assert decoder.eos_token_id == 2, f"Expected 2, got {decoder.eos_token_id}"
...
def test_eos_comparison_works():
    ...
    other_token = 5
    assert other_token != decoder.eos_token_id, "Non-EOS token should not equal EOS"
...
def test_empty_list_fallback():
    ...
    assert decoder.eos_token_id is None, "Empty list should result in None"
...
def test_eos_token_stripped_from_output():
    ...
    assert "<|im_end|>" not in result_text, "EOS token should be stripped from output"
    assert result_text == "Hello", f"Expected 'Hello', got '{result_text}'"
